<%@ page import="org.example.demo.dto.speciality.SpecialityDtoResponse" %>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const specialities = [];
        <%
            List<SpecialityDtoResponse> specs = (List<SpecialityDtoResponse>) request.getAttribute("specialities");
            if(specs != null){
                for(SpecialityDtoResponse s : specs){
        %>
        specialities.push({id: '<%= s.getId() %>', nom: '<%= s.getNom() %>'});
        <%
                }
            }
        %>
        ...
        roleSelect.addEventListener('change', function() {
            ...
            if (role === 'DOCTOR') {
                ...
                let options = '<option value="">Sélectionnez votre spécialité</option>';
                specialities.forEach(spec => {
                    options += `<option value="${spec.id}">${spec.nom}</option>`;
                });
            ...
            `;
                // Ajouter les options dynamiquement via JS
                const specialitySelect = document.getElementById('speciality');
                specialities.forEach(spec => {
                    const opt = document.createElement('option');
                    opt.value = spec.id;
                    opt.textContent = spec.nom;
